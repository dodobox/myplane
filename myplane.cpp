...
int main(){
    ...
    CCanvas* _pCanvas = CCanvas::CreateCanvas( ECT_CONSOLECAVNAS, 100, 110 );
    CStageManager* _pStageManager = CStageManager::GetInterface();
    _pStageManager->ResetStage( 0 );
    _pPlaneManager->AddPlane( EPT_TYPE0, 50, 10, EPD_N, EBT_KEYBOARD, EPC_RED );
    
    while( 1 ){
        _pStageManager->Update( 0 );
